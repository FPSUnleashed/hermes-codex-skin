...
<div data-tree-group="other" id="other"><div data-zone-tabstrip="other"><div role="tablist">${tab('unrelated','Other panel',true)}</div></div></div>
<script>
...
const dark={tab:cs(by('tab1')).backgroundColor,address:cs(by('address')).backgroundColor,border:cs(by('address')).borderTopColor,divider:cs(by('toolbar')).borderBottomColor,tabRadius:cs(by('tab1')).borderRadius,tabHeight:rect(by('tab1')).height,stripHeight:rect(by('tab1').closest('[data-zone-tabstrip]')).height,closeShadow:cs(by('tab1').querySelector('button')).boxShadow,labelCase:cs(by('tab1').querySelector('.truncate')).textTransform,unrelatedRadius:cs(by('unrelated')).borderRadius};
...
root.dataset.hermesMode='light';theme(themes.colors);const light={address:cs(by('address')).backgroundColor,tab:cs(by('tab1')).backgroundColor};
...
window.addEventListener('load',()=>{const doc=by('guest').contentDocument;document.title=btoa(JSON.stringify({sizes,dark,count,pressed,invalid,focus,light,glass,disabled,guest:{background:cs(doc.body).backgroundColor,color:cs(doc.body).color,text:doc.body.textContent}}))});
</script></body></html>`
  try {
    ...
    assert.ok(r.dark.closeShadow.startsWith('rgba(0, 0, 0, 0)'),r.dark.closeShadow)
